...
int main(int argc, char ** argv)
{
    ...
    // initialising Qwidget for morsecode mode
    {
        ...
        lineedit->setPlaceholderText("Enter message you wish to encode and press return key:");
        QVBoxLayout* vblayout = new QVBoxLayout(morsecode);
        vblayout->addWidget(lineedit);

        // connecting QLineEdit signal retrun pressed with desired action in lambda
        QObject::connect(lineedit, &QLineEdit::returnPressed, [=] () {
            // using gpio4 statically as of nof for testing purposes TODO rest
            // qDebug() << "I still live\n";
            GPIOClass gpio4 = GPIOClass(); //gpio4 is default
            gpio4.export_gpio();
            gpio4.setdir_gpio("out");
            // QString text_entered = lineedit->text();
            // qDebug() << text_entered << '\n';
            // qDebug() << lineedit->text() << '\n';
            /*
            for (int i = 0; i < lineedit->text().size(); i++) {
                qDebug() << lineedit->text().at(i) << "\n";
            }
            */
            for (int i = 0; i < lineedit->text().size(); i++) {
                gpio4.morsechar(lineedit->text().at(i));
            }
